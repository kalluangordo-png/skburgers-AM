...
import { formatCurrency, printOrderTicket } from '../../utils';
    ...
    <div className="space-y-8 animate-in fade-in duration-700 pb-20">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-[0.3em]">Gestão Sênior</p>
        </div>
        <button 
        ...
        <StatCard 
          ...
          subtitle={
            <div className="flex flex-col gap-1">
              <span>{config.storeAddress ? `${config.storeAddress}, ${config.storeNumber}` : 'Endereço não configurado'}</span>
              <a 
                href={`https://www.google.com/maps/search/?api=1&query=${config.storeCoords?.lat},${config.storeCoords?.lng}`}
                target="_blank"
                rel="noreferrer"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 glass p-8 rounded-[3rem] min-h-[450px] flex flex-col">
          <h3 className="text-white font-black text-lg uppercase mb-8 italic flex items-center gap-2">
            <ShieldCheck size={20} className="text-yellow-500" /> Performance Diária
          </h3>
        ...
        {/* Últimos Pedidos */}
        <div className="lg:col-span-1 glass p-8 rounded-[3rem] flex flex-col">
          <h3 className="text-white font-black text-lg uppercase mb-6 italic flex items-center gap-2">
            <ShoppingCart size={20} className="text-yellow-500" /> Últimos Pedidos
          </h3>
          <div className="space-y-4 flex-1 overflow-y-auto no-scrollbar max-h-[400px]">
            {orders.slice(0, 5).map(order => (
              <div key={order.id} className="bg-black/20 border border-white/5 p-4 rounded-2xl flex justify-between items-center group">
                <div className="flex items-center gap-3">
                  <button 
                    onClick={() => printOrderTicket(order.id)}
                    className="p-2 bg-zinc-800 text-zinc-500 hover:text-white rounded-xl transition-all opacity-0 group-hover:opacity-100"
                    title="Imprimir"
                  >
                    <Printer size={14} />
                  </button>
                  <div>
                    <p className="text-white font-black italic text-sm">#{order.numeroComanda}</p>
                    <p className="text-[9px] text-zinc-500 font-bold uppercase truncate max-w-[100px]">{order.customerName}</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-yellow-500 font-black text-xs">{formatCurrency(order.total)}</p>
                  <p className="text-[8px] text-zinc-600 font-bold uppercase">{order.status.replace('_', ' ')}</p>
                </div>
            ...
            {orders.length === 0 && (
              <div className="flex flex-col items-center justify-center py-10 text-center opacity-30">
                ...
                <p className="text-[10px] font-black uppercase">Nenhum pedido</p>
              </div>
            )}
          </div>
        </div>
