...
import { 
  Search, Clock, Star, Printer, MessageSquare, Loader2, PackageCheck 
} from 'lucide-react';
...
const AdminOrders: React.FC<AdminOrdersProps> = ({ orders }) => {
  ...
  const filteredOrders = useMemo(() => {
    ...
    return orders.filter(o => {
      ...
      const isToday = orderDate >= startOfOperationalDay;
      const matchesSearch = o.cliente.nome.toLowerCase().includes(searchTerm.toLowerCase()) || 
            ...
            <div key={order.id} className={`bg-zinc-900/40 border-2 rounded-[2rem] p-6 transition-all duration-500 flex flex-col md:flex-row items-center gap-6 ${isLate ? 'border-red-500/30 bg-red-500/[0.02]' : 'border-white/5'}`}>
              ...
              <div className="flex-1 text-center md:text-left">
                ...
                <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest mt-1">
                  {order.cliente.bairro || 'RETIRADA'} • {order.itens.length} ITENS • {order.pagamento}
                  {(order as any).trocoPara && (
                    <span className="text-yellow-500 ml-2 animate-pulse">
                      [TROCO PARA {(order as any).trocoPara}]
                    </span>
                  )}
                </p>
              </div>
              ...
              <div className="flex items-center gap-3 border-t md:border-t-0 md:border-l border-white/5 pt-4 md:pt-0 md:pl-6">
                ...
                <button 
                  onClick={() => sendWhatsAppStatus(order, 'recebido')}
                  className="p-4 bg-blue-500/10 text-blue-500 hover:bg-blue-500 hover:text-black rounded-2xl transition-all border border-blue-500/20"
                  title="Notificar Recebimento"
                >
                ...
                <button 
                  onClick={() => {
                    let type: any = 'recebido';
                    if (order.status === OrderStatus.PREPARING) type = 'preparo';
                    if (order.status === OrderStatus.READY) type = 'preparo'; // Notifica que está pronto
                    if (order.status === OrderStatus.DELIVERING) type = 'entrega';
                    if (order.status === OrderStatus.COMPLETED) type = 'concluido';
                ...
                {order.status === OrderStatus.PENDING && (
                  <button 
                    onClick={() => handleUpdateStatus(order.id, OrderStatus.PREPARING, "Pedido em preparo!")}
                  ...
                  >
                    {processingId === order.id ? <Loader2 className="animate-spin" size={18}/> : 'Iniciar'}
                  </button>
                )}
