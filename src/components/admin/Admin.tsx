...
const Admin: React.FC = () => {
  ...
  const handleFixAddress = async () => {
    if (!config?.cep && !config?.storeAddress) {
      showToast("Preencha o endereço ou CEP da loja!", "error");
      return;
    }

    setIsSaving(true);
    try {
      // Tenta geocodificar o endereço completo da loja
      const fullAddress = `${config.storeAddress || ''}, ${config.storeNumber || ''}, ${config.storeNeighborhood || ''}, CEP ${config.cep || ''}`;
      const queryStr = `${fullAddress}, Manaus, AM, Brasil`;
      ...
      if (lat && lng) {
        ...
        showToast("Localização da Sede atualizada!", "success");
      } else {
        // Tenta apenas pelo CEP se o endereço completo falhar
        if (config.cep) {
          const cepQueryStr = `${config.cep}, Manaus, AM, Brasil`;
          const cepQuery = encodeURIComponent(cepQueryStr);
          let cLat, cLng;

          if (apiKey) {
            const response = await fetch(`https://maps.googleapis.com/maps/api/geocode/json?address=${cepQuery}&key=${apiKey}`);
            const data = await response.json();
            if (data.status === 'OK' && data.results.length > 0) {
              cLat = data.results[0].geometry.location.lat;
              cLng = data.results[0].geometry.location.lng;
            }
          }

          if (!cLat || !cLng) {
            const cepResponse = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${cepQuery}&limit=1`);
            const cepData = await cepResponse.json();
            if (cepData && cepData.length > 0) {
              cLat = parseFloat(cepData[0].lat);
              cLng = parseFloat(cepData[0].lon);
            }
          }
          
          if (cLat && cLng) {
            await setDoc(doc(db, 'config', 'store'), { storeCoords: { lat: cLat, lng: cLng } }, { merge: true });
            showToast("Localização atualizada pelo CEP!", "success");
            return;
          }
        }
        showToast("Não foi possível localizar o endereço da loja.", "error");
      }
    } catch (e) {
      showToast("Erro ao atualizar coordenadas.", "error");
    } finally {
      setIsSaving(false);
    }
  };
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        try {
          ...
          showToast("Sede localizada na sua posição atual!", "success");
        } catch (e) {
