...
import { Clock, Play, CheckCircle, Star, IceCream, AlertCircle, Flame, ChefHat, MessageSquare, Printer } from 'lucide-react';
import * as Firestore from 'firebase/firestore';
const { doc, serverTimestamp, runTransaction, increment, updateDoc } = Firestore as any;
import { db } from '../../services/firebase';
import { useToast } from '../ToastContext';
import { printOrderTicket, getAddonEmoji, sendWhatsAppStatus } from '../../utils';
    ...
    <div className="space-y-6 animate-in fade-in duration-500 pb-20">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          ...
          <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-[0.3em] mt-1">Cozinha em Tempo Real</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          ...
          return (
            <div key={order.id} className={`bg-zinc-900 border-2 rounded-[2.5rem] flex flex-col overflow-hidden transition-all duration-500 ${isLate ? 'border-red-600/50 bg-red-950/10' : 'border-white/5'}`}>
              ...
              <div className="p-5 bg-black/40 flex gap-2">
                <button 
                  onClick={() => printOrderTicket(order.id)}
                  className="p-5 bg-zinc-800 text-zinc-400 hover:text-white rounded-[1.5rem] border border-white/5 transition-all"
                  title="Imprimir Via"
                >
                  <Printer size={20} />
                </button>
                <button 
                  onClick={() => sendWhatsAppStatus(order, order.status === OrderStatus.PENDING ? 'preparo' : 'concluido')}
                  className="p-5 bg-zinc-800 text-emerald-500 rounded-[1.5rem] border border-emerald-500/20 hover:bg-emerald-500 hover:text-black transition-all"
                ...
                <button 
                  ...
                  className={`flex-1 py-5 rounded-[1.8rem] font-black text-xs uppercase tracking-widest flex items-center justify-center gap-3 transition-all active:scale-95 shadow-xl
