...
const Menu: React.FC<MenuProps> = ({ onBack, config }) => {
  ...
  const handleGetLocation = useCallback(async (addressFallback?: string) => {
    ...
    const tryGeocode = async (queryStr: string) => {
      ...
      try {
        ...
        if (e.name === 'AbortError') {
          console.warn("Geocoding timeout para:", queryStr);
        } else {
    ...
    try {
      if (addressFallback) {
        // Tentativa 1: Rua + Número + Bairro + CEP
        let coords = await tryGeocode(`${formData.endereco}, ${formData.numeroCasa}, ${formData.bairro}, ${formData.cep}`);
        
        // Tentativa 2: Rua + Número + Bairro
        if (!coords) {
        ...
        if (coords) {
          const controller = new AbortController();
          const routeTimeout = setTimeout(() => controller.abort(), 5000); // Aumentado para 5 segundos
          ...
          try {
            ...
            if (osrmData.routes && osrmData.routes.length > 0) {
              ...
              const distance = calculateDistance(storeLat, storeLng, coords.lat, coords.lon);
  ...
  }, [isLocating, formData.endereco, formData.numeroCasa, formData.bairro, formData.cep, config.storeCoords]);
  ...
  useEffect(() => {
    ...
    if (checkoutStep === 'form' && formData.endereco.length > 5 && formData.numeroCasa && !isLocating) {
      const timer = setTimeout(() => {
        handleGetLocation(fullAddress);
      }, 1200);
      return () => clearTimeout(timer);
    }
  }, [formData.endereco, formData.numeroCasa, formData.bairro, formData.cep, checkoutStep, handleGetLocation, isLocating]);
              ...
              {featuredCombos.map((p) => (
                ...
                >
                  ...
                  <div className="p-6 space-y-2 -mt-10 relative z-20">
                    ...
                    <p className="text-zinc-500 text-[10px] font-bold line-clamp-2 leading-relaxed">{p.description}</p>
