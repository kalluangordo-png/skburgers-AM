...
const Menu: React.FC<MenuProps> = ({ onBack, config }) => {
  ...
  const availableDrinks = useMemo(() => {
    let drinks = allProducts
      .filter(p => (p.category || '').toUpperCase().includes('BEBIDA'))
      .map(p => p.name);

    if (selectedProductForDrink?.product?.drinkCategory) {
      const filter = selectedProductForDrink.product.drinkCategory.toUpperCase();
      drinks = drinks.filter(d => d.toUpperCase().includes(filter));
    }
  ...
  const handleAddToCartClick = (product: Product) => {
    ...
    // Se a categoria NÃO estiver na lista de categorias de combo, adiciona individual direto
    if (!comboCategories.includes(category)) {
      addToCart({
        ...product,
        price: product.price || product.priceCombo || 0,
        isCombo: false
      });
      return;
    }

    // Se estiver na lista de combos, aplica a lógica de upgrade
  ...
  const handleUpgradeToCombo = (itemId: string) => {
    ...
    if (item && product) {
      const totalPrice = product.priceCombo || (item.price + surcharge);

      if (product.fixedDrink) {
