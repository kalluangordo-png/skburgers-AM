...
import { 
  ...
  CheckCircle2, X, Minus, Trash2, Flame, Utensils, Rocket, Zap
} from 'lucide-react';
...
const Menu: React.FC<MenuProps> = ({ onBack, config }) => {
  ...
  const isOutOfRange = useMemo(() => {
    ...
    const rules = config.deliveryRules || [{ maxKm: 5.5, price: 9.00 }];
  ...
  const handleGetLocation = useCallback(async (addressFallback?: string) => {
    ...
    if (userCep === storeCep && userCep.length > 0) {
      setUserDistance(0.1);
      setIsLocating(false);
      return;
    }
    ...
    const tryGeocode = async (queryStr: string) => {
      ...
      try {
        ...
        if (data && data.length > 0) {
          ...
          // Isso evita que um endereço mal geocodificado aponte para outra cidade ou país.
    ...
    try {
      if (addressFallback) {
        // Tentativa 1: CEP (Geralmente o mais rápido e preciso para localização geral)
        let coords = null;
        if (formData.cep && formData.cep.length >= 8) {
          coords = await tryGeocode(formData.cep);
        }

        // Tentativa 2: Endereço Completo (Se o CEP falhar ou não existir)
        ...
        if (coords) {
          const controller = new AbortController();
          const routeTimeout = setTimeout(() => controller.abort(), 3000); // 3 segundos para rota
          ...
          try {
            ...
            if (osrmData.routes && osrmData.routes.length > 0) {
              ...
              const straightDistance = calculateDistance(storeLat, storeLng, coords.lat, coords.lon);
              
              // Sanity Check: Se a distância de rua for absurdamente maior que a linha reta (> 2.5x),
              // provavelmente o OSRM está fazendo um retorno maluco por erro de mapa ou via bloqueada.
              // Em Manaus isso é comum. Usamos a linha reta com um acréscimo de 20% para ser justo.
              if (roadDistance > straightDistance * 2.5 && straightDistance < 10) {
                setUserDistance(straightDistance * 1.2);
              } else {
                setUserDistance(roadDistance);
              }
            } else {
              setUserDistance(calculateDistance(storeLat, storeLng, coords.lat, coords.lon));
            }
          } catch (e) {
            setUserDistance(calculateDistance(storeLat, storeLng, coords.lat, coords.lon));
          } finally {
      ...
      // Fallback para Geolocation se geocoding falhar ou não houver endereço
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
          (position) => {
            const distance = calculateDistance(storeLat, storeLng, position.coords.latitude, position.coords.longitude);
  ...
  }, [isLocating, formData.endereco, formData.numeroCasa, formData.bairro, formData.cep, config.storeCoords, geocodingCache]);
  ...
  const handleFinalizeOrder = async () => {
    ...
    if (userDistance === null && (!formData.endereco || formData.endereco.length < 5)) {
      showToast("Por favor, preencha o endereço para calcular a entrega.", "error");
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-zinc-950 pb-32 relative">
      ...
      <div 
        ...
        style={{
          backgroundImage: 'url("https://i.postimg.cc/hvWDv5zF/logo-01.jpg")',
      ...
      <div className="relative z-10">
        ...
        <div className="flex justify-between items-center mb-4 md:mb-6">
          <div className="flex items-center gap-3 md:gap-4">
            ...
            <div>
              <h1 className="text-lg md:text-xl font-black tracking-tighter">CARDÁPIO</h1>
              <p className="text-[8px] md:text-[10px] text-zinc-500 font-bold uppercase tracking-wider">SK BURGERS PREMIUM</p>
            </div>
          </div>
                  ...
                  <div className="flex flex-col justify-between flex-1">
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        {p.priceCombo && (
                          <span className="bg-emerald-500/10 text-emerald-500 text-[7px] font-black px-1.5 py-0.5 rounded-full border border-emerald-500/20 uppercase tracking-widest">Combo Disponível</span>
                        )}
                      </div>
                    ...
                    <div className="flex justify-between items-center mt-2">
                      <div className="flex flex-col leading-none">
                        <span className="text-yellow-500 font-black text-base italic">{formatCurrency(p.price)}</span>
                        {p.priceCombo && (
                          <span className="text-[8px] font-black text-zinc-600 uppercase tracking-widest mt-0.5">Combo por {formatCurrency(p.priceCombo)}</span>
                        )}
                      </div>
                      <button 
        ...
        {isCartOpen && (
          ...
          >
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-3">
                      ...
                      <textarea 
                        ...
                        className="w-full bg-zinc-900 border border-white/5 p-5 rounded-2xl text-white font-black text-xs outline-none focus:border-yellow-500 transition-all h-24 resize-none"
                      />
            ...
            {checkoutStep !== 'success' && cart.length > 0 && (
              <footer className="p-4 bg-zinc-950 border-t border-white/10 flex flex-col gap-4">
            ...
            >
              <div className="p-6 space-y-6">
                ...
                <div className="space-y-6">
                  ...
                  <div className="pt-6 border-t border-white/5">
                    <div className="flex items-center justify-between mb-3">
                      <p className="text-[10px] font-black uppercase text-zinc-500 tracking-widest">Oferta Especial</p>
                      <div className="px-2 py-0.5 bg-emerald-500/10 text-emerald-500 text-[8px] font-black rounded-full uppercase">Economize</div>
                    </div>
                    <div className="bg-gradient-to-br from-zinc-800 to-zinc-900 p-5 rounded-[2rem] border border-white/5 relative overflow-hidden group">
                      <div className="absolute -right-4 -top-4 w-20 h-20 bg-yellow-500/10 rounded-full blur-2xl group-hover:bg-yellow-500/20 transition-all" />
                      ...
                      <div className="relative z-10">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <p className="text-sm font-black text-white uppercase italic tracking-tighter">Transformar em Combo?</p>
                        </div>
                        <p className="text-[10px] text-zinc-400 leading-relaxed">
                          Adicione <span className="text-white font-bold">Batata Frita Crocante</span> + <span className="text-white font-bold">Bebida Gelada</span> por apenas <span className="text-yellow-500 font-black text-xs ml-1 tracking-tighter">+ R$ 12,00</span>
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                <div className="space-y-3 pt-4">
                  ...
                  >
                    <Zap size={18} fill="currentColor" /> QUERO O COMBO COMPLETO
                  </button>
                  <button 
                    onClick={() => {
                      const addonTotal = selectedAddons.reduce((acc, a) => acc + a.price, 0);
                    ...
                    className="w-full bg-zinc-800 text-zinc-400 py-5 rounded-[2rem] font-black uppercase text-[11px] tracking-widest hover:bg-zinc-700 hover:text-white transition-all active:scale-95 flex items-center justify-center gap-2 border border-white/5"
                  >
